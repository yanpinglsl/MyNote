...
![image-20230216165601359](images/笔记/image-20230216165601359.png)
...
![image-20230316184052666](images/笔记/image-20230316184052666.png)
...
- **Session共享阶段---Redis**
  ...
  ![image-20230411183012331](images/笔记/image-20230411183012331.png)

- **分布式的去中心化阶段---Token**

  ![image-20230411183039079](images/笔记/image-20230411183039079.png)

  ![image-20230411183058294](images/笔记/image-20230411183058294.png)
...
- Token泄露，安全问题
  ...
  ①SSL通信，传输中途不会窃取；但是如果别人通过浏览器窃取，则无法防止泄露（Cookie或者其他方案都是如此）
...
- Token能做到滑动过期吗
  ...
  ![image-20230507103347929](images/笔记/image-20230507103347929.png)
...
##### JWT实战
  ...
  JWT字符串解析类JWTTokenDeserialize.cs
              ...
              builder.Services.AddAuthentication
                  ...
                  .AddJwtBearer(options =>
                  {
                      options.TokenValidationParameters = new TokenValidationParameters
                      {
                          ...
                          //IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(tokenOptions.SecurityKey))//验证对称可逆加密
                          IssuerSigningKey = new RsaSecurityKey(keyParams)//验证非对称可逆加密
                      };
                  });
              #endregion
  ```
...
![image-20230507202552110](images/笔记/image-20230507202552110.png)
...
![image-20230302140848596](images/笔记/image-20230302140848596.png)

如果服务对外开放，针对Windows系统建议使用IIS+Kestrel；针对Linux系统，建议使用Nginx+Kestrel。

![image-20230302140907316](images/笔记/image-20230302140907316.png)
...
- POP面向过程，没有封装没有边界，难以应对复杂需求

- OOP面向对象，有封装有边界，能应付复杂需求，

  但原子(类)不容破坏，否则会影响稳定性---开闭原则

  AOP能在不破坏封装的前提下，去额外扩展功能。比如ActionFilter。

##### NetCore中AOP

- 中间件：离MVC比较远，面向与请求级的事儿
- Filter：更贴近MVC，面向单个请求,更细一些
- IOC容器的AOP扩展：细化到具体业务，跟分层封装相关（例如AutoFac）

#### FIlter注册方式

自定义Filter

- Action：直接标记特性---只对方法生效

- Controller：控制器标记特效型-----控制器下全部Action生效

- 全局：在AddMVC时添加到Options-----进程内全部Action生效

- ModelConvention（参考《ModelConventions模型约定》部分）
...
- 控制器类实现Filter接口
  ...
  ```C#
  public class FilterController : Controller, IActionFilter, IAsyncActionFilter
  ...
  > 一个Filter里面既有同步，又有异步，只执行异步！

#### Filter执行顺序

> 只有继承IOrderedFilter接口才能自定义Filter执行顺序

- Order默认0，相同注册的Filter是从小到大执行，可以为负数
- 相同Order相同注册，从先注册到后注册
- 全局---控制器---Action
...
- 通过TypeFilter(typeof(Filter)) 标记在方法，标记在控制器
  ...
  ```C#
  	   [TypeFilter(typeof(CustomDIActionFilterAttribute))]
...
- 通过ServiceFilter(typeof(Filter))标记在方法，标记在控制器，必须要注册Filter这类；
  ...
  ```C#
  	//Program.cs
   	builder.Services.AddTransient<CustomDIActionFilterAttribute>();
  	   ...
  	   //COntroller.cs
  	    [ServiceFilter(typeof(CustomDIActionFilterAttribute))]
          public IActionResult DI()
          {
              this._logger.LogWarning($"This is {nameof(FilterController)}-DI LogWarning");
...
- 自定义实现IFilterFactory
  ...
  CustomIOCFilterFactoryAttribute.cs
...
>
> IFilterFactory接口中IsReusable参数：是否支持Filter重用，默认为true，如果为false，则每次调用都会重新创建不再重用
