> 原文链接：[C#枚举高级战术 - 精致码农 - 博客园 (cnblogs.com)](https://www.cnblogs.com/willick/p/csharp-enum-superior-tactics.html)
...
> 建议：在 C# 编程中，当使用 Flags 特性定义枚举类型时，建议从 1 开始。这是因为枚举值可以通过按位或运算符 | 进行组合，以表示一组选项。如果枚举值的值是 0 或 -1，那么它们就无法通过按位或运算符进行组合。
