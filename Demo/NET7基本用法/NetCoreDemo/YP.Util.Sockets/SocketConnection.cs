    ...
    public class SocketConnection
    {
                    ...
                    try
                    {
                        ...
                        if (length > 0)
                        {
                            ...
                            try
                            {
                                ...
                                Task.Run(() => HandleRecMsg?.Invoke(recBytes, this, _server));
                            }
                            catch (Exception ex)
                            {
                                //HandleException?.BeginInvoke(ex, null, null);
                                Task.Run(() => HandleException?.Invoke(ex));
                            }
                        }
                        else
                            Close();
            ...
            try
            {
                _socket.BeginSend(bytes, 0, bytes.Length, SocketFlags.None, asyncResult =>
                {
                    try
                    {
                        ...
                        Task.Run(() => HandleSendMsg?.Invoke(bytes,this, _server));
                    }
                    catch (Exception ex)
                    {
                        //HandleException?.BeginInvoke(ex, null, null);
        ...
        public void Close()
        {
            try
            {
                ...
                Task.Run(() => HandleClientClose?.Invoke(this, _server));
            }
            catch (Exception ex)
            {
                //HandleException?.BeginInvoke(ex, null, null);
